...
def test_execute_blank_password(mock_session, mock_uow, user_builder):
    user = user_builder()
...
def test_execute_bad_password(mock_session, mock_uow, user_builder):
    user = user_builder()
    mock_uow.users.get_by_username.return_value = user
    service = GetUser(session=mock_session, unit_of_work=lambda _: mock_uow)
    with pytest.raises(BadCredentialsException):
...
def test_execute_correct_password(mock_session, mock_uow, user_builder):
    user = user_builder()
    mock_uow.users.get_by_username.return_value = user
    service = GetUser(session=mock_session, unit_of_work=lambda _: mock_uow)

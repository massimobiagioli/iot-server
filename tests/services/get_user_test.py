...
def test_execute_blank_password(mock_db, user_data):
    mock_db.exec.return_value.first.return_value = user_data
...
def test_execute_bad_password(mock_db, user_data):
    mock_db.exec.return_value.first.return_value = user_data
    service = GetUser(mock_db)
    with pytest.raises(BadCredentialsException):
...
def test_execute_correct_password(mock_db, user_data):
    mock_db.exec.return_value.first.return_value = user_data
    service = GetUser(mock_db)
